# expire_cache_storage_plug — Wiki Schema

Wiki schema for the `expire_cache_storage_plug` package: a **storage plug** for the Pluggable plug-n-play framework. It implements the framework's `PluggableStorageProvider` contract using the `expire_cache` package.
...
`expire_cache_storage_plug` provides in-memory storage with per-entry time-based expiry. Its single public class, `ExpireCacheStoragePlug`, extends `PluggableStorageProvider`, so it can be slotted into a `PluggableStorage` as the `local` or `remote` provider. Storage is non-persistent; entries also expire after a configured duration.
...
├── raw/
├── narrative/
│   ├── modules/        # expire-cache-storage-plug.md
...
- Backend: the `expire_cache` package.
- Dependencies: `expire_cache`, `synchronized`, Flutter SDK, `pluggable`.
