...
related: [narrative/modules/expire-cache-storage-plug, narrative/concepts/storage-contract]
covers_packages: [expire_cache_storage_plug]
---

# Using the Storage Plug

`ExpireCacheStoragePlug` plugs into the framework as a storage provider. It is suitable for short-lived caches where stale data should self-evict.
...
await initPluggable(
  storagePlugin: PluggableStorage(
    local: ExpireCacheStoragePlug(),
    remote: ExpireCacheStoragePlug(),
  ),
  modules: [/* ... */],
);
```
...
- Persistence: non-persistent; entries also expire after a configured duration.
- For the exact method signatures, see [structure/manifest.json](../../structure/manifest.json).

## See Also

- [[narrative/modules/expire-cache-storage-plug]] — backend mapping.
