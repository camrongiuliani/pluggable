# expire_cache_storage_plug — Overview

`expire_cache_storage_plug` is a storage plug for the Pluggable plug-n-play framework. It provides in-memory storage with per-entry time-based expiry by implementing the framework's `PluggableStorageProvider` contract over the `expire_cache` package.

A "plug" satisfies an abstract contract so it can be swapped without changing application code. `ExpireCacheStoragePlug` can be used as the `local` and/or `remote` provider inside a `PluggableStorage`. Suitable for short-lived caches where stale data should self-evict.
...
- [[narrative/modules/expire-cache-storage-plug]] — the `ExpireCacheStoragePlug` class and how it maps the contract onto its backend.
...
| "What backend does this use?" | `narrative/modules/expire-cache-storage-plug` |
