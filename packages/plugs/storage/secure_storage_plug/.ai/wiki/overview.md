# secure_storage_plug — Overview

`secure_storage_plug` is a storage plug for the Pluggable plug-n-play framework. It provides persistent encrypted on-device storage by implementing the framework's `PluggableStorageProvider` contract over encrypted Hive (via `stash` + `stash_hive`).

A "plug" satisfies an abstract contract so it can be swapped without changing application code. `SecureStoragePlug` can be used as the `local` and/or `remote` provider inside a `PluggableStorage`. Suitable for storing sensitive data (tokens, credentials) that must be encrypted at rest.
...
- [[narrative/modules/secure-storage-plug]] — the `SecureStoragePlug` class and how it maps the contract onto its backend.
...
| "What backend does this use?" | `narrative/modules/secure-storage-plug` |
