# secure_storage_plug — Wiki Schema

Wiki schema for the `secure_storage_plug` package: a **storage plug** for the Pluggable plug-n-play framework. It implements the framework's `PluggableStorageProvider` contract using encrypted Hive (via `stash` + `stash_hive`).
...
`secure_storage_plug` provides persistent encrypted on-device storage. Its single public class, `SecureStoragePlug`, extends `PluggableStorageProvider`, so it can be slotted into a `PluggableStorage` as the `local` or `remote` provider. Storage is persistent and encrypted at rest.
...
├── raw/
├── narrative/
│   ├── modules/        # secure-storage-plug.md
...
- Backend: encrypted Hive (via `stash` + `stash_hive`).
- Dependencies: `stash`, `stash_hive`, `uuid`, `synchronized`, Flutter SDK, `pluggable`.
